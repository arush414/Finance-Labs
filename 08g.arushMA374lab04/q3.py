...
def visualize_data(x_values, y_values, x_axis_label, y_axis_label, plot_title):
...
def visualize_curves(x1, y1, x2, y2, x_axis_label, y_axis_label, plot_title):
    plt.plot(x1, y1, color='Blue', label='Minimum Variance Curve')
    plt.plot(x2, y2, color='Green', label='Capital Market Line (CML)')
    plt.xlabel(x_axis_label)
    plt.ylabel(y_axis_label)
    plt.title(plot_title)
    plt.grid(True)
...
def main():
  ...
  visualize_curves(risk, returns, risk_cml, returns_cml, "Risk (sigma)", "Returns", "Capital Market Line with Markowitz Efficient Frontier")
  visualize_data(risk_cml, returns_cml, "Risk (sigma)", "Returns", "Capital Market Line")

  print("\n\n==================  sub-part (d) ==================\n")
  stocks_data = [ "RELIANCE",
        "TCS",
        "INFY",
        "WIPRO",
        "MARUTI",
